...
let getVerticesAndIndicesSmoothNormals orientation sphere =
    ...
    let vertices =
        sphere.Vertices
        |> Array.map (fun vertex -> VertexPositionNormalColour(vertex, Vector3.Normalize(vertex * factor), Color.Yellow))
    let indices =
        sphere.Faces
...
let getVerticesAndIndicesFlatNormals orientation sphere =
    ...
    let (vertices, indices) =
        ...
        |> Array.mapi (fun n face ->
            ...
            [|
                (VertexPositionNormalColour(vertex1, cross, Color.Red), 3 * n);
                (VertexPositionNormalColour(vertex2, cross, Color.Red), 3 * n + 1);
                (VertexPositionNormalColour(vertex3, cross, Color.Red), 3 * n + 2);
            |])
        |> Array.concat
        |> Array.unzip
    (vertices, indices)
